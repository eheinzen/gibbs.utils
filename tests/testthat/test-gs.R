test_that("Gibbs works for 1-D cases", {
  ...
  mu8 <- gs_diagmatlm_beta(beta = 1.1, y = one, X = one, V = ten, U = NULL, mu0 = 0, Q0 = one, params.only = TRUE)
...
test_that("Conjugacy works for 2-D independent cases", {
  ...
  mu8 <- gs_diagmatlm_beta(beta = c(1.1, 1.65), y = one, X = cbind(X, X), V = ten, U = NULL, mu0 = zero, Q0 = two, params.only = TRUE)
