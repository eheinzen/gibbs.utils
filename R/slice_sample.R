...
#'
#'   This is vectorized over \code{L}, \code{k}, and \code{mean}. If \code{precsision} is a matrix,
...
#' @export
ss_poisson_regression <- function(L, k, mean, precision) {
  if(length(L) != length(k)) stop("'L' and 'k' must all have the same length")
